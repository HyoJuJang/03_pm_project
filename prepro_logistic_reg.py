...
pretrain1['epdo']= np.dot(dsci, [12, 5, 3, 1])
...
d_flat
d_flat['week'] = d_flat['Week'].apply(make_week)
df['week']
d_flat['week']
print((df['week'].reset_index(drop=True) == d_flat['week'].reset_index(drop=True)).sum())
df2 = pd.concat([df, d_flat[['X','Y','SOILSLOPE','yes_count']].reset_index(drop=True)], axis=1)
df_org2 = df.copy()
df2
df = df2
df['sidewalks'] = df['yes_count'].replace(np.nan, 0)
df['sidewalks'].value_counts()
df2[df2['sidewalks']>=7]
def make_sidewalks(x):
    if x == 0:
        ...
        return 1
df['sidewalks_y'] = df['sidewalks'].apply(make_sidewalks)
df = df.drop('yes_count', axis=1)
df
plt.hist(df['at_age'])
plt.show()
plt.hist(df[df['at_age']>85]['at_age'])
plt.show()
### 100세 이상 삭제
df = df[(df['at_age']<=100) & (df['vt_age']<=100)]
df['vt_age_bin'] = bin_age(df['vt_age']).astype('int')
df['at_age_bin'] = bin_age(df['at_age']).astype('int')
col = df.columns
...
#one_hot_encoding_cate(df[['SOILSLOPE']])
df.tail()
df = df.loc[:,[(df[col].dtype != 'object') for col in df.columns]]
df
#df.to_csv(save_path + "df_v1.csv", encoding="EUC-KR", index=False)
dt = df.drop(labels=['hour','accident','d_acci','s_acci','c_acci','i_acci','at_age','vt_age','at_age_bin',
                     'vt_age_bin','at_acci','vt_acci','alch','elder','cycle','cross','child',
                    'vt_gender','at_gender','alch_cont', 
                     'road_linear', 'road_straight', 'road_flat',
                     'X', 'Y'
                    ],axis=1)
dt
dt_org = dt.copy()
...
#dt.to_csv(save_path + "df_v2.csv", encoding="EUC-KR", index=False)
...
pt1_col1 = dt_vp.columns[4:9]
pt1_col2 = dt_vp.columns[11:17]
pt1_col3 = dt_vp.columns[-8:]
print(pt1_col1)
print(pt1_col2)
print(pt1_col3)
pt1 = dt_vp[pt1_col1]
...
clf.best_params_
lr1 = LogisticRegression(C=5, verbose=5)
...
lr2 = LogisticRegression(C=8, verbose=5)
...
lr3 = LogisticRegression(C=9, verbose=5)
...
get_ipython().run_line_magic('save', 'prepro_logistic_reg.py 1-300')
