...
df_vv_test.drop(['accident','at_age','vt_age'],axis=1,inplace=True)
df_vv_train.drop(['accident','at_age','vt_age'],axis=1,inplace=True)
...
df_vv_train_mm = df_vv_train.drop('class',axis=1)
df_vv_test.columns == df_vv_train_mm.columns
from sklearn.preprocessing import MinMaxScaler
minMaxScaler = MinMaxScaler()
minMaxScaler.fit(df_vv_train_mm)
df_vv_train_minMaxScaled = minMaxScaler.transform(df_vv_train_mm)
df_vv_test_minMaxScaled = minMaxScaler.transform(df_vv_test)
df_vv_train_minmax = pd.DataFrame(df_vv_train_minMaxScaled)
df_vv_train_minmax.columns = df_vv_train.drop('class',axis=1).columns
df_vv_train_minmax.describe()
df_vv_test_minmax = pd.DataFrame(df_vv_test_minMaxScaled)
df_vv_test_minmax.columns = df_vv_test.columns
df_vv_test_minmax.describe()
df_vv_train_minmax
df_vv_train
df_vv_train_minmax.reset_index(drop=True, inplace=True)
df_vv_train.reset_index(drop=True, inplace=True)
df_vv_train_minmax
df_vv_train
df_vv_train_minmax['class'] = df_vv_train['class']
df_vv_train['class'].value_counts()
df_vv_train_minmax['class'].value_counts()
df_vv_train_minmax.columns
df_vv_test_motor = df_vv_test.drop(list(df_vv_test.filter(regex = '._motor')), axis = 1)
df_vv_train_motor = df_vv_train.drop(list(df_vv_train.filter(regex = '._motor')), axis = 1)
df_vv_test_minmax_motor = df_vv_test_minmax.drop(list(df_vv_test_minmax.filter(regex = '._motor')), axis = 1)
df_vv_train_minmax_motor = df_vv_train_minmax.drop(list(df_vv_train_minmax.filter(regex = '._motor')), axis = 1)
df_vv_train_minmax_motor
df_vv_test_motor.columns
from sklearn.metrics import accuracy_score
from sklearn.metrics import confusion_matrix, plot_confusion_matrix
from sklearn.metrics import roc_curve, auc, roc_auc_score
from pycaret.classification import *
df_vv_train_minmax_motor
df_vv_train_minmax_motor.isnull().sum()
clf2 = setup(data =df_vv_train_minmax_motor, target = 'class', fix_imbalance=True,
...
predictions = predict_model(final_model2, data = df_vv_test_minmax_motor)
...
len(final_model2.coef_[0])
len(df_vv_train_minmax_motor.columns)
fi0 = pd.DataFrame({'Feature': get_config('X_train').columns, 'Value' : lr_coef1[0]})
fi = pd.concat([fi0,pd.DataFrame(lr_coef1[1]),pd.DataFrame(lr_coef1[2])],axis=1).sort_values(by='Value', key=abs,ascending=False)
fi.columns = ['feature','byc','pm','motor']
fi_2
fi0 = pd.DataFrame({'Feature': get_config('X_train').columns, 'Value' : final_model2_coef1[0]})
fi = pd.concat([fi0,pd.DataFrame(lr_coef1[1]),pd.DataFrame(lr_coef1[2])],axis=1).sort_values(by='Value', key=abs,ascending=False)
fi.columns = ['feature','byc','pm','motor']
fi_2
fi0 = pd.DataFrame({'Feature': get_config('X_train').columns, 'Value' : final_model2_coef[0]})
fi = pd.concat([fi0,pd.DataFrame(lr_coef1[1]),pd.DataFrame(lr_coef1[2])],axis=1).sort_values(by='Value', key=abs,ascending=False)
fi.columns = ['feature','byc','pm','motor']
fi_2
final_model2.coef_
...
fi_2
fi0 = pd.DataFrame({'Feature': get_config('X_train').columns, 'Value' : final_model2.coef_[0]})
fi = pd.concat([fi0,pd.DataFrame(final_model2.coef_[1]),pd.DataFrame(final_model2.coef_[2])],axis=1).sort_values(by='Value', key=abs,ascending=False)
fi.columns = ['feature','byc','pm','motor']
fi_2
fi0 = pd.DataFrame({'Feature': get_config('X_train').columns, 'Value' : final_model2.coef_[0]})
...
fi_2
fi_2.sum(axis=1)
df_vv_train_minmax_motor.columns
df_vv_train_minmax_motor_inter = df_vv_train_minmax_motor.copy()
df_vv_test_minmax_motor_inter = df_vv_test_minmax_motor.copy()
col_driving = np.array(['at_driving_straight','at_driving_leftright',
                        'law_viol_byc','law_viol_pm',
                       'at_activ_byc', 'at_activ_pm'])
col_env = np.array(['road_straight_byc','road_straight_pm',
                    'road_form_B','cross_form_byc', 'cross_form_pm',
                    'road_num','tree_n', 'subway_n'])
col_slope = dt_vv2.columns[dt_vv2.columns.str.startswith('slope')]

dt_vv2 = df_vv_train_minmax_motor_inter
...
dt_vv2
dt_vv2.columns
df_vv_train_minmax_motor_inter = dt_vv2
dt_vv2 = df_vv_test_minmax_motor_inter
...
for col1 in col_driving:
    for col2 in col_slope:
        dt_vv2[col2 + '*' + col1] = dt_vv2[col1].mul(dt_vv2[col2])
df_vv_test_minmax_motor_inter = dt_vv2
df_vv_train_minmax_motor_inter.columns
len(df_vv_train_minmax_motor_inter.columns) - len(df_vv_test_minmax_motor_inter.columns)
clf3 = setup(data =df_vv_train_minmax_motor_inter, target = 'class', fix_imbalance=True,
            numeric_features=['road_num','busstop_n','tree_n','subway_n'], feature_selection=True)
...
predictions = predict_model(final_model3, data = df_vv_test_minmax_motor)
df_vv_test_minmax_motor.columns
df_vv_test_minmax_motor_inter
predictions = predict_model(final_model3, data = df_vv_test_minmax_motor_inter)
pred_label=predictions['Label']
pred_score=predictions['Score']
pred_label.value_counts()
print(accuracy_score(label_test_y , pred_label))
label_test_y
label_test_y == pred_label
label_test_y
label_test_y.iloc[:,0]
label_test_y.iloc[:,1]
label_test_y.iloc[1]
label_test_y
label_test_y.iloc[1,1]
label_test_y.iloc[1]
label_test_y
pred_label
pred_label.reset_index(drop=True)
label_test_y(drop=True)
pred_label.reset_index(drop=True) == label_test_y
pd.Series(pred_label) == label_test_y
pd.Series(pred_label) == pd.Series(label_test_y)
pd.Series(pred_label.reset_index(drop=True)) == pd.Series(label_test_y.reset_index(drop=True))
(pd.Series(pred_label.reset_index(drop=True)) == pd.Series(label_test_y.reset_index(drop=True))).sum()
(pd.Series(pred_label.reset_index(drop=True)) == pd.Series(label_test_y.reset_index(drop=True)))
(pd.Series(pred_label.reset_index(drop=True)) == pd.Series(label_test_y.reset_index(drop=True))).sum()/3264
...
fi_3.sum(axis=1)
df_vv_train_minmax_motor.columns
df_vv_train_minmax_motor.drop(['at_prot', 'vt_prot', 'alch', 'at_gender', 'vt_gender'],axis=1)
df_vv_train_minmax_motor2 = df_vv_train_minmax_motor.drop(['at_prot', 'vt_prot', 'alch', 'at_gender', 'vt_gender'],axis=1)
df_vv_test_minmax_motor2 = df_vv_test_minmax_motor.drop(['at_prot', 'vt_prot', 'alch', 'at_gender', 'vt_gender'],axis=1)
df_vv_train_minmax_motor_inter2 = df_vv_train_minmax_motor_inter.drop(['at_prot', 'vt_prot', 'alch', 'at_gender', 'vt_gender'],axis=1)
df_vv_test_minmax_motor_inter2 = df_vv_test_minmax_motor_inter.drop(['at_prot', 'vt_prot', 'alch', 'at_gender', 'vt_gender'],axis=1)
clf2_2 = setup(data =df_vv_train_minmax_motor2, target = 'class', fix_imbalance=True,
            numeric_features=['road_num','busstop_n','tree_n','subway_n'], )
best_m2_2 = compare_models()
lr2_2 = create_model('lr')
evaluate_model(lr2_2)
tuned_lr2_2 = tune_model(lr2_2)
final_model2_2 = finalize_model(tuned_lr2_2)
evaluate_model(final_model2_2)
predictions = predict_model(final_model2_2, data = df_vv_test_minmax_motor2)
pred_label=predictions['Label']
pred_score=predictions['Score']
pred_label.value_counts()
label_test_y.value_counts()
print(accuracy_score(label_test_y , pred_label))
fi0 = pd.DataFrame({'Feature': get_config('X_train').columns, 'Value' : final_model2_2.coef_[0]})
fi_2_2 = pd.concat([fi0,pd.DataFrame(final_model2_2.coef_[1]),pd.DataFrame(final_model2_2.coef_[2])],axis=1).sort_values(by='Value', key=abs,ascending=False)
fi_2_2.columns = ['feature','byc','pm','motor']
fi_2_2
df_vv_train_minmax_motor2.columns
df_vv_train_minmax_motor_inter2.columns
len(df_vv_train_minmax_motor_inter2.columns) - len(df_vv_test_minmax_motor_inter2.columns)
clf3_2 = setup(data =df_vv_train_minmax_motor_inter2, target = 'class', fix_imbalance=True,
            numeric_features=['road_num','busstop_n','tree_n','subway_n'], feature_selection=True)
best_m3_2 = compare_models()
lr3_2 = create_model('lr')
#evaluate_model(lr3_2)
tuned_lr3_2 = tune_model(lr3_2)
final_model3_2 = finalize_model(tuned_lr3_2)
evaluate_model(final_model3_2)
predictions = predict_model(final_model3_2, data = df_vv_test_minmax_motor_inter2)
pred_label=predictions['Label']
pred_score=predictions['Score']
pred_label.value_counts()
label_test_y.value_counts()
print(accuracy_score(label_test_y , pred_label))
fi0 = pd.DataFrame({'Feature': get_config('X_train').columns, 'Value' : final_model3_2.coef_[0]})
fi_3_2 = pd.concat([fi0,pd.DataFrame(final_model3_2.coef_[1]),pd.DataFrame(final_model3_2.coef_[2])],axis=1).sort_values(by='Value', key=abs,ascending=False)
fi_3_2.columns = ['feature','byc','pm','motor']
fi_3_2.head(30)
interpret_model(final_model3_2)
get_ipython().run_line_magic('save', 'modeling_with_new_testtrain.py 1-300')
